...
\begin{example}
  The free group generated by the singleton set $\set{\star}$ is the group of
  integers $(\ZZ, 0, {+}, {-})$. The map $\eta : \set{\star} \to \ZZ$ takes
  $\star$ to $0$. As an exercise you should verifty that the integers have the
...
\begin{example}
  ...
  set~$\one = \set{\star}$: the sum of $t_1$ and $t_2$ is written as
  %
  \begin{equation*}
    \mathsf{add}(\star; t_1, t_2).
  \end{equation*}
  %
...
of Gordon Plotkin and John Power~\cite{plotkin-power} was that many
computational effects are naturally described by algebraic theories.
