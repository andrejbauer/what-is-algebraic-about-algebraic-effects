...
\newcommand{\opcall}[4]{\kode{#1}(#2; #3.\,#4)}
...
etc. We may model such operations mathematically as operations on an algebra
whose elements are \emph{computations}. A computation either
%
\begin{enumerate}
\item terminates and returns a \emph{value}, or
\item performs an operation and passes its result to the \emph{continuation}.
\end{enumerate}
%
(We are ignoring a third possibility, non-termination.) It is best to consider
an example:
%
\begin{equation*}
  \opcall{read}{\unit}{s}{
  \opcall{print}{\text{\texttt{"Hello "}} \mathbin{{+}{+}} s}{\_}{
  \return{42}}
  }.
\end{equation*}


Leaving the exact nature of~$C$ unspecified for
a moment, let us look at how computational effects correspond to operations
on~$C$.

\begin{example}
  The operation $\mathtt{print}$ takes a string~$s$ as an argument, sends~$s$ to
  the standard output, and returns control to the program.


  Printing to the standard output is a unary operation parametrized by strings,
  so a map
  %
  \begin{equation*}
    \mathtt{print} : \mathtt{string} \times C^\one \to C.
  \end{equation*}
  % 
  The term $\mathtt{print}(s, \kappa)$ corresponds to the program which prints the
  string~$s$, and activates the computation $\kappa$ by passing it the result of
  printing (which is necessarily the only element of~$\one$). We call $\kappa$ the
  \emph{continuation} because it tells us how the program continues after it has
  printed the string~$s$.
\end{example}

\begin{example}
  Reading from the standard input is an operation whose parameter is trivial,
  and whose arity is $\mathtt{string}$,
  %
  \begin{equation*}
    \mathtt{read} : \one \times C^{\mathtt{string}} \to C.
  \end{equation*}
  %
  The term $\mathtt{read}(\star, \kappa)$ corresponds to the program which reads a
  string~$s$ from the standard input and passes it to the rest of the
  computation~$\kappa$.
\end{example}
